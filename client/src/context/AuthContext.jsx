...
const AuthProvider = ({ children }) => {
  const [authData, setAuthData] = useState(() => {
    ...
    try {
      return storedAuthData ? JSON.parse(storedAuthData) : { token: null, userId: null,username:null };
    } catch (error) {
  ...
  useEffect(() => {
    if (authData.token && authData.userId,authData.username) {
  ...
  const logout = () => {
    ...
    setAuthData({ token: null, userId: null,username:null });
  };
